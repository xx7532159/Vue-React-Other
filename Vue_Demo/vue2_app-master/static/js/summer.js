...
//            }
  ...
  return {
   ...
   s.addEventListener = function (json, successFn, errFn) {
     if (s.canrequire()) { return s.cordova.require('summer-plugin-frame.XFrame').addEventListener(json, successFn, errFn) }
   }
 ...
 +(function (w, s) {
   ...
   s.UMService = {
    ...
    return
        ...
        alert('Excp5.2: 异步调用adrinvoker.call异常:' + e)
         ...
         var info = ''
         if (isSync) { info = '调用$service.call("' + serviceType + '", jsonArgs, ' + isSync + ')时发生异常,请检查!' } else					{ info = '调用$service.call("' + serviceType + '", jsonArgs)时发生异常,请检查!' }
         console.log(info)
   ...
   s.UMCamera = {
     ...
     openPhotoAlbum: function (json) {
       if (!json) return
       var args = {}
       if (json.bindfield) { args['bindfield'] = json['bindfield'] }
       if (json.callback) { args['callback'] = json['callback'] }
       if (json.compressionRatio) { args['compressionRatio'] = json['compressionRatio'] }
